...
public class Chapter1Solutions {
   ...
   * 
   * Time complexity: O(min(c,n)) where c is number of characters in alphabet (256)
   * and n is the length of the string. Or O(1) technically.
   * Space complexity: O(c) where c is number of characters in alphabet (256). Or O(1) technically.
   */
   ...
   * 
   * Assumptions: Characters are ASCII characters, capitalization matters, strings aren't larger than 2^16.
   * 
   * Time complexity: O(max(c,n) where c is number of characters in alphabet (256)
  ...
  public static boolean arePermutations(String a, String b) {
    ...
    if (a.length() != b.length()) {
      ...
      short[] occurances = new short[256];
